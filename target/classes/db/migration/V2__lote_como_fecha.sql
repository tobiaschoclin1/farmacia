-- Agrego fecha_lote (ISO yyyy-MM-dd) y mantengo numero_lote por compatibilidad
ALTER TABLE lotes ADD COLUMN fecha_lote TEXT;

-- Para datos existentes, seteo hoy si está null
UPDATE lotes SET fecha_lote = strftime('%Y-%m-%d','now') WHERE fecha_lote IS NULL;
...
SELECT
  ...
  CASE WHEN p.unidades_por_caja IS NOT NULL AND p.unidades_por_caja > 0
    THEN CAST(COALESCE(sl.cantidad_base,0) / p.unidades_por_caja AS INT)
