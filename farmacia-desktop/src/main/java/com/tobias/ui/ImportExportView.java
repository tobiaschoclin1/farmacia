...
public class ImportExportView extends BorderPane {
  ...
  private final Button btnExportTodo = new Button("Exportar Inventario");
  private final Button btnImportar   = new Button("Importar Datos");
  ...
  public ImportExportView() {
    ...
    var iconExport = Icons.arrowDownOnSquare(16);
    iconExport.setStyle("-fx-background-color: white;");
    btnExportTodo.setGraphic(iconExport);
    btnExportTodo.getStyleClass().add("primary");

    var iconImport = Icons.arrowUpTray(16);
    iconImport.setStyle("-fx-background-color: #2563EB;");
    btnImportar.setGraphic(iconImport);
    btnImportar.getStyleClass().add("secondary");
  ...
  private Node buildHeader() {
    var title = new Label("Importar / Exportar Datos");
    title.getStyleClass().add("page-title");

    var subtitle = new Label("Exporta e importa tu inventario en formato Excel (.xlsx)");
  ...
  private Node buildExportCard() {
    var card = new VBox(16);
    card.getStyleClass().add("card");

    var cardTitle = new Label("Exportar Inventario");
    cardTitle.getStyleClass().add("card-title");
    cardTitle.setStyle("-fx-font-size: 18px;");

    var description = new Label(
      "Exporta todo el inventario a un archivo Excel con dos hojas:\n" +
      "  • Productos: Vista consolidada con stock total por producto\n" +
      "  • Lotes: Detalle de cada lote con fechas y cantidades"
    );
    description.setWrapText(true);
    description.setStyle("-fx-text-fill: #6B7280;");

    var featuresBox = new VBox(8);
    featuresBox.setPadding(new Insets(12));
    featuresBox.setStyle("-fx-background-color: #F3F4F6; -fx-background-radius: 8;");

    var feat1 = new Label("✓ Incluye todos los productos activos e inactivos");
    var feat2 = new Label("✓ Desglose completo por lotes y vencimientos");
    var feat3 = new Label("✓ Calculos automaticos de cajas y tabletas");
    var feat4 = new Label("✓ Formato compatible con Excel y Google Sheets");

    feat1.setStyle("-fx-text-fill: #374151;");
    feat2.setStyle("-fx-text-fill: #374151;");
    feat3.setStyle("-fx-text-fill: #374151;");
    feat4.setStyle("-fx-text-fill: #374151;");

    featuresBox.getChildren().addAll(feat1, feat2, feat3, feat4);

    btnExportTodo.getStyleClass().add("primary");
    btnExportTodo.setPrefWidth(300);

    var btnContainer = new HBox(btnExportTodo);
    btnContainer.setAlignment(Pos.CENTER_LEFT);
    btnContainer.setPadding(new Insets(8, 0, 0, 0));

    card.getChildren().addAll(cardTitle, description, featuresBox, btnContainer);
    return card;
  }

  private Node buildImportCard() {
    var card = new VBox(16);
    card.getStyleClass().add("card");

    var cardTitle = new Label("Importar Datos");
    cardTitle.getStyleClass().add("card-title");
    cardTitle.setStyle("-fx-font-size: 18px;");

    var description = new Label(
      "Importa productos y lotes desde un archivo Excel (.xlsx) previamente exportado.\n" +
      "El archivo debe contener las hojas 'Productos' y 'Lotes' con el formato correcto."
    );
    description.setWrapText(true);
    description.setStyle("-fx-text-fill: #6B7280;");

    var warningBox = new VBox(8);
    warningBox.setPadding(new Insets(12));
    warningBox.setStyle("-fx-background-color: #FEF3C7; -fx-background-radius: 8; -fx-border-color: #F59E0B; -fx-border-width: 1; -fx-border-radius: 8;");

    var warningTitle = new Label("⚠️ Importante");
    warningTitle.setStyle("-fx-font-weight: 700; -fx-text-fill: #92400E;");

    var warn1 = new Label("• La importacion actualiza productos existentes y crea nuevos");
    var warn2 = new Label("• Los lotes se identifican por producto + fecha de lote");
    var warn3 = new Label("• El stock de cada lote se REEMPLAZA con el valor importado");
    var warn4 = new Label("• Verifica los datos antes de importar para evitar perdidas");

    warn1.setStyle("-fx-text-fill: #92400E; -fx-font-size: 13px;");
    warn2.setStyle("-fx-text-fill: #92400E; -fx-font-size: 13px;");
    warn3.setStyle("-fx-text-fill: #92400E; -fx-font-size: 13px;");
    warn4.setStyle("-fx-text-fill: #92400E; -fx-font-size: 13px;");

    warningBox.getChildren().addAll(warningTitle, warn1, warn2, warn3, warn4);

    btnImportar.getStyleClass().add("warning");
    btnImportar.setPrefWidth(300);

    var btnContainer = new HBox(btnImportar);
    btnContainer.setAlignment(Pos.CENTER_LEFT);
    btnContainer.setPadding(new Insets(8, 0, 0, 0));

    card.getChildren().addAll(cardTitle, description, warningBox, btnContainer);
    return card;
  }
  ...
  private void exportarInventario() {
    try {
      ...
      String[] hTot = {"Codigo","Producto","Unidad","Unid/Caja","Cant. base total","Cajas total","Tabletas total","Stock min."};
      ...
      // Hoja Lotes
      String[] hLot = {"Codigo","Producto","Fecha lote","Vencimiento","Cant. base","Cajas"};
      ...
      new Alert(Alert.AlertType.INFORMATION,
        ...
        "Archivo: " + f.getName() + "\n" +
        "Productos exportados: " + rowsTot.size() + "\n" +
        "Lotes exportados: " + rowsLot.size(),
  ...
  private void importarXlsx() {
    try {
      ...
      var confirm = new Alert(Alert.AlertType.CONFIRMATION,
        "Esta accion actualizara los datos existentes.\n\n" +
        "Estas seguro de continuar?",
        ButtonType.YES, ButtonType.NO);
      confirm.setTitle("Confirmar Importacion");
      confirm.setHeaderText("Importar datos desde " + f.getName());
      ...
      new Alert(Alert.AlertType.INFORMATION,
        "Importacion finalizada exitosamente!\n\n" +
